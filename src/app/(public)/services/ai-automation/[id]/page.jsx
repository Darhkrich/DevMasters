  ...
  useEffect(() => {
    ...
    const loadItem = async () => {
      try {
        ...
        try {
          result = await fetchAIAutomation(params.id);
        } catch (automationError) {
          result = await fetchAIBundle(params.id);
          itemType = "bundle";
        }
        ...
        setRelatedItems(
          itemType === "automation"
            ? aiAutomations.filter((entry) => entry.id !== params.id && entry.sector === normalizedItem.sector).slice(0, 3)
      ...
      <div className="wc-ai-not-found">
        <div style={{ textAlign: "center", padding: "50px 20px" }}>
          ...
          <p style={{ marginBottom: "20px" }}>
            The automation or bundle with ID &quot;{params?.id}&quot; was not found.
          </p>
          <Link
